...
#define FINSH_WORD_COLOUR "\033[1;32;40m"
#define FINSH_WORD_RESET "\033[0m"
...
static void shell_auto_complete(char *prefix)
{
    ...
    rt_kprintf("%s%s%s%s",FINSH_WORD_COLOUR, FINSH_PROMPT, prefix,FINSH_WORD_RESET);
}
...
static rt_bool_t shell_handle_history(struct finsh_shell *shell)
{
    ...
    rt_kprintf("\033[2K");
#endif
    rt_kprintf("%s%s%s%s",FINSH_WORD_COLOUR, FINSH_PROMPT, shell->line,FINSH_WORD_RESET);
    return RT_FALSE;
}
...
void finsh_thread_entry(void *parameter)
{
    ...
    rt_kprintf("%s%s%s",FINSH_WORD_COLOUR, FINSH_PROMPT,FINSH_WORD_RESET);
    fflush(stdout);
    while (1)
    {
        ch = finsh_getchar();
        ...
        else if (shell->stat == WAIT_FUNC_KEY)
        {
            ...
            else if (ch == 0x43) /* right key */
            {
                ...
                {
                    rt_kprintf("%s%c%s",FINSH_WORD_COLOUR, shell->line[shell->line_curpos],FINSH_WORD_RESET);
                    shell->line_curpos ++;
                }
        ...
        else if (ch == 0x7f || ch == 0x08)
        {
            ...
            if (shell->line_position > shell->line_curpos)
            {
                ...
                rt_kprintf("\b%s%s%s  \b",FINSH_WORD_COLOUR, &shell->line[shell->line_curpos],FINSH_WORD_RESET);
        ...
        if (shell->line_curpos < shell->line_position)
        {
            ...
            if (shell->echo_mode)
                rt_kprintf("%s%s%s",FINSH_WORD_COLOUR, &shell->line[shell->line_curpos],FINSH_WORD_RESET);// print the char what we input
        ...
        {   
            ...
            if (shell->echo_mode)
                rt_kprintf("%s%c%s",FINSH_WORD_COLOUR, ch, FINSH_WORD_RESET);
        }
...
int finsh_system_init(void)
{
...
#ifdef FINSH_USING_SYMTAB
    /* GNU GCC Compiler */
    rt_kprintf("finsh_system_init case __GUNC__\n");
    ...
    ret = pthread_create(&m_thread_finsh, &attr, finsh_thread_entry, NULL);
    rt_kprintf("thread finsh create %d\n",ret);

    sem_init(&(shell->rx_sem),0,1);
